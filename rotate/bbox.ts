...
let drawMode = new class {
    drawFromPoint: null|Point2D = null;
    entermode() {
    ...
    pointermove(evt: PointerEvent) {
        if (this.drawFromPoint == null) return;
        ctx.beginPath();
...
class Mat3x2 {
    ...
    public static rotation(theta: number) {
        return new Mat3x2([
            Math.cos(theta), -Math.sin(theta), 0,
            Math.sin(theta),  Math.cos(theta), 0,
        ]);
    }
    public static translation(x: number, y: number) {
        return new Mat3x2([
            1, 0, x,
            0, 1, y,
        ]);
    ...
    public static shearX(l: number) {
        return new Mat3x2([
            1, l, 0,
            0, 1, 0,
        ]);
    }
    public static shearY(l: number) {
        return new Mat3x2([
            1, 0, 0,
            l, 1, 0,
        ]);
    }
    public mul(other: Mat3x2): Mat3x2 {
        let m = this.m;
        let o = other.m;
        return new Mat3x2([
            m[0] * o[0] + m[1] * o[3],
    ...
    public transformPt(pt: Point2D): Point2D {
        let m = this.m;
        return new Point2D(
    ...
    public transformDir(pt: Point2D): Point2D {
        let m = this.m;
        return new Point2D(
            m[0] * pt.x + m[1] * pt.y,
            m[3] * pt.x + m[4] * pt.y
...
class ToolTransform {
    ...
    rotation: number = 0;
    scaleAndShear: Mat3x2 = Mat3x2.identity();
    origin: Point2D = new Point2D(0, 0);
    ...
    getComposedTransform(): Mat3x2 {
        return (
            Mat3x2.translation(this.translation.x, this.translation.y)
...
let transformMode = new class TransformMode {
    ...
    cageBounds: {low: Point2D, high: Point2D}|null = null;

    cageRotation: number = Math.PI / 6;
    prevCageRotation: number = this.cageRotation;
    ...
    canvasToCageTransform: Mat3x2 = Mat3x2.rotation(this.cageRotation);
    cageToCanvasTransform: Mat3x2 = this.canvasToCageTransform.inverse();

    imageData: ImageData;
    supportMap: SupportMap|null = null;

    entranceTransform = new ToolTransform();
    prevTransform = new ToolTransform();
    transform = new ToolTransform();

    cageHandles: {[name: string]: {at: Point2D, path: Path2D}} = {};

    pathHandle(at: Point2D) {
        let path = new Path2D();
        path.rect(at.x - 5, at.y - 5, 10, 10);
        return {at, path};
    }
    pathRect(a: Point2D, b: Point2D) {
        let path = new Path2D();
    ...
    drawHandle(handle: typeof this.cageHandles[string]) {
    ...
    bestFitAngle(): number {
        if (this.supportMap == null) return 0.0;
        ...
        let scoreFn: (bounds: {min: Point2D, max: Point2D}) => number;
        let scoreByMinArea: typeof scoreFn = bounds => -(bounds.max.x - bounds.min.x) * (bounds.max.y - bounds.min.y);
        let scoreByLongestAxis: typeof scoreFn = bounds => Math.max(bounds.max.x - bounds.min.x, bounds.max.y - bounds.min.y);
        ...
        for (let angle = 0.0; angle < Math.PI / 2.0; angle += Math.PI / 180.0) {
            ...
            let toPoint = ({x, y}: {x: number, y: number}) => new Point2D(x, y);
    ...
    computeBounds(transform: Mat3x2) {
        if (this.supportMap == null) return null;
    ...
    pointInHandle(handleName: string, x: number, y: number) {
    ...
    setCtxTransform(ctx: CanvasRenderingContext2D, mat: Mat3x2) {
    ...
    cagePath() {
        let path = new Path2D();
        if (this.cageBounds == null) return path;
    ...
    setCageRotation(radians: number) {
    ...
    entermode() {
        ...
        if (pixels != null) {
            this.supportMap = new SupportMapSum(
                pixels,
                new SupportMapPoints([{x: 0, y: 0}, {x: 1, y: 0}, {x: 0, y: 1}, {x: 1, y: 1}])
            );
        } else {
    ...
    dragMode: "cage-rotate"|"scale-x"|"scale-y"|"scale-both"|"shear-x"|"shear-y"|"rotate"|"translate"|"origin"|"none" = "none";
    dragModeExtraInfo = {
        grabbedHandleName: "",
        anchorName: "",
    };
    startDrag: Point2D|null = null;
    lastMousePos: Point2D = new Point2D(0, 0);
    debugCageSpacePoints: Point2D[] = [];
    debugCanvasSpacePoints: Point2D[] = [];

    scaleRelationships: {grab: string, anchor: string, mode: "scale-both"|"scale-x"|"scale-y"|"shear-x"|"shear-y"}[] = [
    ...
    pointermove(evt: PointerEvent) {
        if (this.startDrag != null) {
            ...
            if (this.dragMode === "cage-rotate") {
                ...
                else if (this.dragMode === "scale-x") sy = 1;
                else if (this.dragMode === "scale-y") sx = 1;
                this.transform.scaleAndShear = (
                    this.prevTransform.scaleAndShear
...
type ToolMode = (typeof drawMode) | (typeof transformMode);
let currentMode: ToolMode = drawMode;
