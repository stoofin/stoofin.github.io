...
declare const jumpToTimeCheckbox: HTMLInputElement;
declare const specificTimeTooltip: HTMLDivElement;
        ...
        listen(spanDiv, {
            mousemove(evt: MouseEvent) {
                if (jumpToTimeCheckbox.checked) {
                    ...
                    if (!is_placeholder) {
                        (spanDiv as HTMLAnchorElement).href = "https://twitch.tv/videos/" + video.id + makeTimeArgument(subDate(hoverDate, video.start));
                    }
                } else {
                    (spanDiv as HTMLAnchorElement).href = url;
                }
            },
            mouseleave() {
...
function getInterestedChannels(): "follows" | string[] {
...
async function initial() {
    ...
    let streamsPromise: Promise<Map<string, TwitchStream>> = Promise.resolve(new Map());
    ...
    vodRequestQueue.addTasks(users.filter(user => !channelVODTasks.has(user.id)).map(user =>
        makeChannelVODTask(user.id, 0)
    ));
